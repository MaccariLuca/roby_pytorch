...
# FIX: scipy.ndimage.interpolation è deprecato, usare scipy.ndimage
...
class GaussianNoise(Alteration):
    ...
    def apply_alteration(self, data: np.ndarray,
        ...
        if not(-0.000001 <= float(alteration_level) <= 0.000001):
            ...
            data = (data + gauss).astype(np.float32) 
            
            # To active when work with  0-255 image and you need a clip
        ...
        if alteration_level != 0.0:
            if isinstance(data, np.ndarray):
                if self.picture_mode == 'RGB':
                    # Ensure compatible types for PIL
...
class Zoom(Alteration):
    ...
    def apply_alteration(self, data: np.ndarray,
        ...
        # FIX: usare scipy.ndimage.zoom, importato correttamente in alto
